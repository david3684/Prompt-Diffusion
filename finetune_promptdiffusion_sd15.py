...
def main(args):
    ...
    data_module = ControlDataModule(
        ...
        train_tasks=args.train_tasks, #finetuning task
        test_tasks=[], 
    ...
    data_module.setup()
    train_dataloader = data_module.tuning_dataloader(num_supports=15, shots=1, tasks=args.train_tasks)
    ...
    while global_step < args.max_train_steps:
        batch = next(cycle(train_dataloader))  # 무한 반복 데이터 로더에서 배치 가져오기
        with accelerator.accumulate(controlnet):
            ...
            target = target.reshape(B * T, target.shape[2], target.shape[3], target.shape[4])  
