...
def main(args):
    ...
    if args.controlnet_model_name_or_path:
        ...
        controlnet = PromptDiffusionControlNetModel.from_pretrained(args.controlnet_model_name_or_path)
    else:
        logger.info("Initializing controlnet weights from unet")
        controlnet = PromptDiffusionControlNetModel.from_unet(unet)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                ...
                load_model = PromptDiffusionControlNetModel.from_pretrained(input_dir, subfolder="controlnet")
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = [d for d in dirs if d.startswith("checkpoint-") and d[len("checkpoint-"):].isdigit()]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                noisy_latents = noise_scheduler.add_noise(gt_latents.float(), noise.float(), timesteps).to(
                    dtype=weight_dtype
                )

                noisy_latents = noisy_latents.unsqueeze(1).repeat(1, T, 1, 1, 1)  # [B, T, C', H', W']
                ...
                target = target.unsqueeze(1).repeat(1, T, 1, 1, 1)  # [B, T, C', H', W']
                target = target.reshape(B * T, target.shape[2], target.shape[3], target.shape[4])  # [B*T, C', H', W']
